...
2. **Strategy, PRD, and design are already approved — they are your contract.** Read all of these at the repo root before you start and build to them; do not silently drift, and log any forced deviation in `.planning/ASSUMPTIONS.md`:
   ...
   - `design/` + `design/DESIGN-SYSTEM.md` — the approved wireframes and design system (unless `DESIGN-SKIP.txt` exists, meaning no UI). The built UI must match these screens and tokens. (Do NOT re-run design from scratch — it's done and approved.)

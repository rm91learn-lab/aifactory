...
**Apply PM frameworks (required): use the relevant pm-skills.** Don't hand-wave the analysis — invoke the applicable framework skills (via the Skill tool) and fold their output into STRATEGY.md: `value-proposition` and `business-model` (or `lean-canvas`/`startup-canvas`) for the thesis; `ideal-customer-profile` / `user-personas` for who has the problem; `competitor-analysis`, `porters-five-forces`, `swot-analysis`, `market-sizing`, `pestle-analysis` for the landscape and risks; `pricing-strategy` / `monetization-strategy` where money matters; `north-star-metric` for what "good" looks like; `opportunity-solution-tree` to connect the problem to the MVP. Then **stress-test with `strategy-red-team`** before finalizing. Use judgement — apply the ones that fit this product, not all of them.

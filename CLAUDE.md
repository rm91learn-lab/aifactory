...
| 3 | **Design** | gstack `design-consultation` + `design-html` + taste-skills | experience UX loop (fewest clicks, no legacy) + `design-taste-frontend` + the matching aesthetic (`high-end-visual-design`/`minimalist-ui`/`industrial-brutalist-ui`) → premium `design/` wireframes, served at `/preview/<slug>/` | **🚦 human** (UI only; non-UI auto-skips) |
| 4 | Build | GSD + superpowers | per phase: `/gsd:plan-phase` → `autoplan` (CEO/design/eng/DX) → `/gsd:execute-phase` (UI via `image-to-code` from the wireframes + `full-output-enforcement` — no placeholders) → `/gsd:verify-work` + `review-code` + `cso` + `design-review`; **staging only** | auto |
...
- **taste-skills (7)** are the anti-slop frontend layer (`design-taste-frontend`, `high-end-visual-design`, `minimalist-ui`, `industrial-brutalist-ui`, `redesign-existing-projects`, `image-to-code`, `full-output-enforcement`). Used in the Design gate (premium, non-templated wireframes) and Build (faithful wireframe→code, complete output). They add taste judgment on top of the design pipeline — they don't replace `design-consultation`/`design-html`/`design-review`.

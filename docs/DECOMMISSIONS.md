...
## 2026-06-14 — factory-console fully retired (round 2)
A leftover queued build re-promoted factory-console during an unattended window, recreating the worker we'd deleted. Now fully retired: worker deleted again AND the `products/factory-console` workspace removed so the daemon can no longer rebuild it. The dashboard is `scripts/build-dashboard.mjs` served by the daemon; factory-console-as-a-product was redundant. Archive: GitHub `rm91learn-lab/factory-console`.

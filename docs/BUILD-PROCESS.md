...
`design-consultation` sets a design system; `design-html` produces real wireframes of the key screens. The **taste layer** runs here too: `design-taste-frontend` (anti-slop) infers the right direction from the brief, and the matching aesthetic skill (`high-end-visual-design` / `minimalist-ui` / `industrial-brutalist-ui`) makes the result look premium, never templated. For every primary journey the agent runs the experience loop — *deliberate → research → evaluate → deliberate → finalize* — to commit to the **fewest possible clicks with modern patterns and zero legacy clunk** (recorded in `design/UX-RATIONALE.md`). The factory serves the screens at `/preview/<product>/`.
...
The build agent builds strictly to the three approved artifacts. Per phase: `/gsd:plan-phase` (research → plan → machine-verified) → `autoplan` review (CEO → design → eng → DX) → `/gsd:execute-phase` (parallel waves, **failing-test-first**, root-cause-only fixes; UI translated faithfully from the approved wireframes via `image-to-code`, with `full-output-enforcement` so the code ships complete — no placeholders or TODOs) → `/gsd:verify-work` + `review-code` + `cso` (security) + `design-review`. Every action streams to the dashboard. **Deployment here is staging-only** (`DEPLOY-STAGED.json`: preview URL + promote command) — production is unreachable from this stage. (Exception: a product's first-ever deploy, which QA still gates before announcement.)
